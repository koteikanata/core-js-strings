...
function repeatString(str, times) {
  return str.repeat(times > 1 ? times : 0);
}
...
function sumOfCodes(str = '') {
  return typeof str === 'string'
    ? str.split('').reduce((acc, char) => char.charCodeAt() + acc, 0)
    : 0;
}
...
function countVowels(str) {
  const match = str.toLowerCase().match(/[aeiouy]/gi);
...
function isPalindrome(str) {
  return (
    str
      ...
      .split('')
      .reverse()
      .join('') === str.toLowerCase().replace(/[\W\s]/g, '')
  );
}
...
function findLongestWord(sentence) {
  ...
  sentence
    ...
    .forEach((word) => (max = word.length > max.length ? word : max));
  return max;
}
  ...
  return str
    ...
    .map((ch, i) => {
      if (ch.match(/\w/g)) {
        ...
        const start = charNum >= 65 && charNum <= 90 ? 65 : 97;
        return String.fromCharCode(((charNum - start + 13) % 26) + start);
      }
      return ch;
    })
    .join('');
